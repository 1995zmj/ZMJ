...
#include "Customization/TextBlockDataCustomization.h"
...
void FSMSystemEditorModule::StartupModule()
{
	...
	// ZMJ
	PropertyModule.RegisterCustomPropertyTypeLayout(TEXT("TextBlockData"), FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FTextBlockDataCustomization::MakeInstance));
	...
	AssetRemovedHandle = AssetRegistry.OnAssetRemoved().AddRaw(this, &FSMSystemEditorModule::OnAssetRemoved);
...
void FSMSystemEditorModule::ShutdownModule()
{
	...
	// ZMJ
	PropertyModule.UnregisterCustomPropertyTypeLayout(TEXT("FTextBlockData"));
	...
	*/
	...
	if (AssetRemovedHandle.IsValid() && FModuleManager::Get().IsModuleLoaded("AssetRegistry"))
	{
		IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();
...
void FSMSystemEditorModule::OnAssetRemoved(const FAssetData& InAssetData)
{
	if (InAssetData.AssetClass == USMBlueprint::StaticClass()->GetFName())
	{
		InAssetData.PrintAssetData();
		auto Path = FSMBlueprintEditorUtils::GetUSMBlueprintExportJsonPath(Cast<USMBlueprint>(InAssetData.GetAsset()));
		if (FPaths::FileExists(Path))
		{
			IFileManager& TempFileManager = IFileManager::Get();
			TempFileManager.Delete(*Path);
		}
		UE_LOG(LogTemp,Warning,TEXT("removed asset %s"),*InAssetData.GetFullName());
	}

	// auto PathStr = FPaths::ProjectDir() + FSMBlueprintEditorUtils::GetEditorSettings()->ExportJsonPath;
	// FBlockServer testS;
	// testS.ID = 100;
	// testS.Content = TEXT("赵敏将");
	// MyDataTable->AddRow("test", testS);
	...
	// FFileHelper::SaveStringToFile(str,*FilePath, FFileHelper::EEncodingOptions::ForceUTF8);
	/**/
}

void FSMSystemEditorModule::OnAssetRenamed(const FAssetData& InAssetData, const FString& OldName)
{
	UE_LOG(LogTemp, Warning, TEXT("Renamed asset old name %s"), *OldName);
}
