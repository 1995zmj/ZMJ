...
public:
	...
	// ----------ZMJ----------
	// 不支持修改路径，之后可以加，要把原先的文件移走
	UPROPERTY(config, VisibleAnywhere, Category = "Blocks")
    FString ExportCSVPath;

	UPROPERTY(config, VisibleAnywhere, Category = "Blocks")
	FString ExportJsonPath;
    	
	UPROPERTY(config, EditAnywhere, Category = "Blocks")
	FLinearColor BlocksDefaultColor;
