...
void USMGraphSchema::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	FText CategoryState = LOCTEXT("State", "State"); // FText::GetEmpty()

	// Add new state node
	if (bBaseStatesAllowed)
	{
		TSharedPtr<FSMGraphSchemaAction_NewNode> NewNodeAction = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddState", "Add State..."), LOCTEXT("AddStateTooltip", "A new state which contains entry points for logic execution."), BaseGrouping);
		NewNodeAction->NodeTemplate = NewObject<USMGraphNode_StateNode>(ContextMenuBuilder.OwnerOfTemporaries);
	}

	// Add new conduit node
	{
		TSharedPtr<FSMGraphSchemaAction_NewNode> NewNodeAction = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddConduit", "Add Conduit..."), LOCTEXT("AddConduitTooltip", "A new conduit for branching to different states."), BaseGrouping);
	...
	{
		TSharedPtr<FSMGraphSchemaAction_NewNode> NewNodeAction = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddStateMachine", "Add State Machine..."), LOCTEXT("AddStateMachineTooltip", "A new state machine."), BaseGrouping);
	...
	if (bParentsAllowed)
	{
		...
		{
			TSharedPtr<FSMGraphSchemaAction_NewNode> NewNodeAction = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddParentStateMachine", "Add State Machine Parent..."), LOCTEXT("AddParentStateMachineTooltip", "A new state machine from the parent graph."), BaseGrouping);
	...
	// Entry point (only if doesn't already exist) Shouldn't need this since the entry point can't be removed.
	{
		...
		if (!bHasEntry)
		{
			TSharedPtr<FSMGraphSchemaAction_NewNode> Action = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddEntryPoint", "Add Entry Point..."), LOCTEXT("AddEntryPointTooltip", "Define State Machine's Entry Point."), BaseGrouping);
	...
	if (!ContextMenuBuilder.FromPin)
	{
		...
		// Add any state node
		{
			TSharedPtr<FSMGraphSchemaAction_NewNode> NewNodeAction = AddNewStateNodeAction<FSMGraphSchemaAction_NewNode>(ContextMenuBuilder, CategoryState, LOCTEXT("AddAnyState", "Add Any State..."), LOCTEXT("AddAnyStateTooltip", "A special state node that represents any other state within this FSM."), BaseGrouping);
