...
void USMGraphK2Schema::CreateDefaultNodesForGraph(UEdGraph& Graph) const
{
	...
	// ZMJ
	FGraphNodeCreator<USMGraphK2Node_BlockMachineNode> BlockMachineNodeCreator(Graph);
	USMGraphK2Node_BlockMachineNode* BlockMachineNode = BlockMachineNodeCreator.CreateNode();
	BlockMachineNodeCreator.Finalize();
	SetNodeMetaData(BlockMachineNode, FNodeMetadata::DefaultGraphNode);
	BlockMachineNode->NodePosY = 500;
