...
* 高光反射`specular` 发射方向 $$r = 2(\hat{n} \cdot I )\hat{n} - I$$ `Phong`模型 $$c_{spscular} = (c_{light} \cdot m_{specular})max(0,\hat{v}\cdot r)^{m_{gloss}}$$

  $$mgloss$$ 是材质的光泽度，也被称为反光度`shininess` 控制高光区域“亮点”有多宽 $$m_{specular}$$是光源的颜色和强度 `Blinn`模型 $$\hat{h}$$ 通过对$$\hat{v}$$和$$\hat{I}$$的去平均后再归一化得到的
